...
export function spawnAIPlayer(playerIndex, characterIndex, difficulty) {
    ...
    if (difficulty === "BOSS") {
        ...
        ai.use(scale(0.35)); // Reduced again to 0.35 to ensure smooth movement
        // Very tight hitbox to prevent clipping on corners
        ai.use(area({ scale: 0.4, offset: vec2(0, 10) }));
    }
